...
export default class ApprovedPurchasesScreen extends Component<Props> {
  constructor(props){
    super(props);
    this.state = ({
      loader: false,
  ...
  async getData(){
    ...
    let approvedPurchases = await getData('purchases', this.state.currentUserId, 'approved');
    await this.setState({ data: approvedPurchases });
  ...
  render() {
    ...
    return (
      ...
      {this.state.data == false ?
        <Text>No Approved Purchases Found</Text>
        :
