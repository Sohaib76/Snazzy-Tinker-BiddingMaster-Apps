...
export default class ClearedPurchasesScreen extends Component<Props> {
  constructor(props){
    super(props);
    this.state = ({
      loader: false,
  ...
  async getData(){
    ...
    let clearedPurchases = await getData('purchases', this.state.currentUserId, 'cleared');
    await this.setState({ data: clearedPurchases });
  ...
  render() {
    ...
    return (
      ...
      {this.state.data == false ?
        <Text>No Cleared Purchases Found</Text>
        :
